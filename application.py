...
ROLE_UI_ACCESS = {
    ...
    "trader": ["/home", "/tradingview/chart", "/momentum_watchlist", "/trade"],
    "admin": ["/home", "/tradingview/chart", "/momentum_watchlist", "/admin/ui-management", "/trade"]
}
...
@require_role(["trader", "admin"])
def trade_ui():
    return render_template('trade.html', pages=g.ui_pages, role=g.role, user=g.user)
...
def api_get_ltp():
    ...
    symbol = data.get("symbol", "").strip().upper()
    if not symbol:
        return json_response(success=False, message="Missing symbol"), 400
    ltp = get_ltp(symbol)
    return json_response(symbol=symbol, ltp=ltp)
...
def trade():
    if request.method == 'POST':
        # Log raw form data
        logger.debug(f"Trade POST data: {request.form}")
        data = request.form.to_dict()
        symbol = data.get("symbol", "").strip().upper()
        action = data.get("transaction_type", "").upper() or data.get("action", "").upper()
        qty = int(data.get("qty", 0))
        ...
        logger.debug(f"Normalized trade data -> symbol: {symbol}, action: {action}, qty: {qty}, price: {price}, productType: {productType}, order_type: {order_type}, trigger_price: {trigger_price}")

        try:
            # Place order using core function
            ...
            if result.get("success"):
                ...
                flash(f"❌ Order Placement Failed: {result.get('message')}", "danger")
        except Exception as e:
            logger.error(f"Order failed: {e}", exc_info=True)
            flash(f"❌ Order Failed: {str(e)}", "danger")

        # Redirect back to GET /trade so UI can show flash messages
        return redirect(url_for('trade'))

    # GET request -> fetch current orders for dashboard
    open_orders, completed_orders = fetch_orders_from_dhan()
    return render_template("trade.html", open_orders=open_orders, completed_orders=completed_orders)

@app.route('/api/open_orders', methods=['GET'])
@require_role(["trader", "admin"])
def api_open_orders():
    open_orders, _ = fetch_orders()
    return json_response(**{"orders": open_orders})

@app.route('/api/completed_orders', methods=['GET'])
@require_role(["trader", "admin"])
def api_completed_orders():
    _, completed_orders = fetch_orders()
    return json_response(**{"orders": completed_orders})
...
def api_positions():
    ...
    return json_response(success=True, positions=positions)
...
def api_exit_position():
    ...
    open_orders, _ = fetch_orders()
    # exit opposite action at market
    order_to_exit = next((o for o in open_orders if o["symbol"] == symbol), None)
    if not order_to_exit:
        return json_response(success=False, message=f"No open position for {symbol}"), 404

    action = "SELL" if order_to_exit["transaction_type"] == "BUY" else "BUY"
    qty = order_to_exit["qty"]
    price = get_ltp(symbol)
    result = place_order(symbol, action, qty, price, productType="INTRADAY", order_type="MARKET")
    return json_response(**result)

@app.route('/api/exit_position_custom', methods=['POST'])
@require_role(["trader", "admin"])
def api_exit_position_custom():
    data = request.get_json()
    symbol = data.get("symbol", "").strip().upper()
    action = data.get("action", "").upper()
    qty = int(data.get("qty", 0))
